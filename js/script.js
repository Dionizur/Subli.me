...
function renderProducts() {
  ...
  if (products.length === 0) {
    productsGrid.innerHTML = '<div style="text-align:center;padding:60px 0;color:#999;"><p style="font-size:3rem;">👕</p><p>Nenhum produto disponivel no banco de dados.</p><p style="font-size:0.85rem;color:#bbb;margin-top:8px;">Va em Admin e adicione produtos.</p></div>';
    return;
  }
  ...
  $$('.product-card').forEach(function(card) {
    card.addEventListener('click', function() {
      const product = products.find(function(p) { return p.id === card.dataset.id; });
...
function openModal(product) {
  ...
  function render() {
    ...
    if (hasMultiple) {
      html += '<button class="gallery-nav gallery-prev">‹</button><button class="gallery-nav gallery-next">›</button><div class="gallery-dots">' + imgs.map(function(_, i) { return '<span class="gallery-dot ' + (i === idx ? 'active' : '') + '" data-i="' + i + '"></span>'; }).join('') + '</div>';
    }
    html += '</div><div class="modal-details"><div class="categoria">' + (product.categoria || 'Geral') + '</div><h2>' + product.nome + '</h2><div class="preco-modal">R$ ' + parseFloat(product.preco).toFixed(2) + '</div><p class="descricao">' + (product.descricao || '') + '</p><div class="tamanhos-title">Tamanhos:</div><div class="tamanhos-list">' + (product.tamanhos || []).map(function(t) { return '<span class="tam-item" data-tam="' + t + '">' + t + '</span>'; }).join('') + '</div><button class="btn btn-whatsapp" id="whats-btn-modal">📱 Encomendar</button></div>';
    modalContent.innerHTML = html;

    if (hasMultiple) {
      $('.gallery-prev').onclick = function(e) { e.stopPropagation(); idx = (idx - 1 + imgs.length) % imgs.length; render(); };
      $('.gallery-next').onclick = function(e) { e.stopPropagation(); idx = (idx + 1) % imgs.length; render(); };
      $$('.gallery-dot').forEach(function(d) { d.onclick = function(e) { e.stopPropagation(); idx = parseInt(d.dataset.i); render(); }; });
    }

    let selectedSize = null;
    $$('.tam-item').forEach(function(el) {
      el.onclick = function() {
        $$('.tam-item').forEach(function(e) { e.classList.remove('selected'); });
  ...
  adminLoginBtn.onclick = async function() {
    ...
    try {
      ...
      if (user) {
        ...
        adminLoginError.textContent = 'Usuario ou senha incorretos!';
        adminLoginError.style.display = 'block';
        adminPasswordInput.value = '';
        adminPasswordInput.focus();
      }
    } catch (err) {
      adminLoginError.textContent = 'Erro ao autenticar. Verifique o console (F12).';
      adminLoginError.style.display = 'block';
    } finally {
...
if (adminPasswordInput) adminPasswordInput.onkeydown = function(e) { if (e.key === 'Enter') adminLoginBtn.click(); };
if (adminEmailInput) adminEmailInput.onkeydown = function(e) { if (e.key === 'Enter') adminPasswordInput.focus(); };
...
if (fileInput) fileInput.onchange = function() {
  ...
  for (const file of files) {
    ...
    reader.onload = function(e) { pendingImages.push({ name: file.name, dataURL: e.target.result }); updatePreviews(); };
...
function updatePreviews() {
  ...
  previewContainer.innerHTML = '<div class="multi-previews">' + pendingImages.map(function(img, i) { return '<div class="preview-item"><img src="' + img.dataURL + '" alt=""><button class="preview-remove" data-i="' + i + '">X</button></div>'; }).join('') + '</div>';
  $$('.preview-remove').forEach(function(btn) { btn.onclick = function() { pendingImages.splice(parseInt(btn.dataset.i), 1); updatePreviews(); }; });
}
...
if (adminForm) {
  adminForm.onsubmit = async function(e) {
    ...
    const tamanhos = tamanhosRaw.split(',').map(function(t) { return t.trim().toUpperCase(); }).filter(function(t) { return t; });
    if (tamanhos.length === 0) { showToast('Informe tamanhos.', 'warning'); return; }

    let imagens = [];
    if (pendingImages.length > 0) imagens = pendingImages.map(function(img) { return img.dataURL; });
    ...
    const result = await addProduct({ nome: nome, descricao: descricao, preco: preco, categoria: categoria, tamanhos: tamanhos, imagens: imagens, ativo: true });
    if (result) {
      products.push(result);
      renderProducts();
      renderAdminProducts();
      showToast('Produto adicionado ao banco!', 'success');
      adminForm.reset();
      pendingImages = [];
      updatePreviews();
    } else {
...
function renderAdminProducts() {
  ...
  if (products.length === 0) {
    adminProductsList.innerHTML = '<p style="color:#999;">Nenhum produto cadastrado no banco.</p>';
    return;
  }
  adminProductsList.innerHTML = products.map(function(p) {
    const img = getImageSrc(getProductImages(p)[0]);
  ...
  $$('.btn-remove').forEach(function(btn) {
    btn.onclick = async function() {
      if (!confirm('Remover este produto do banco?')) return;
      btn.textContent = '...';
      btn.disabled = true;
...
if (syncBtn) syncBtn.onclick = async function() { showToast('Sincronizando...', 'info'); await loadProducts(); };

// ===== WHATSAPP FLOAT =====
const wFloat = document.querySelector('.whatsapp-float');
if (wFloat) wFloat.onclick = function() { window.open('https://wa.me/' + CONFIG.whatsapp + '?text=' + encodeURIComponent('Ola! Vim do site Subli.me'), '_blank'); };

// ===== INIT =====
async function init() {
  const connected = initSupabase();
  if (!connected) {
    showToast('ERRO: Configure o Supabase em js/supabase-config.js', 'error');
    navigateTo('home');
    return;
  }
